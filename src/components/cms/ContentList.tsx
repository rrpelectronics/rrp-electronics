...
import { Plus, Search, FileText, Calendar as CalendarIcon, Edit3, Trash2 } from "lucide-react";
    ...
    <div className="space-y-8 lg:space-y-12">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 sm:gap-8 border-b border-gray-200 pb-8 sm:pb-12">
        <div className="space-y-2">
          <h1 className="text-body2 font-medium text-gray-900 tracking-tight">{getCategoryLabel()}</h1>
          <p className="text-body4 text-gray-400 font-normal">Configure and index {category} metadata for public delivery.</p>
        </div>
        <button
          onClick={onAdd}
          className="h-11 px-4 sm:px-10 bg-[#FF5C19] text-white rounded-md text-body4 font-medium tracking-tight hover:bg-orange-600 transition-all shadow-md cursor-pointer flex items-center justify-center gap-2 active:scale-95 whitespace-nowrap"
        >
        ...
        <div className="overflow-x-auto min-h-[400px]">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/10">
                <th className={cn("px-4 lg:px-10 py-5 text-body4 font-medium text-gray-500 tracking-tight min-w-[300px]", category === 'careers' && "min-w-[400px]")}>
                  <div className="flex items-center gap-2.5">
                    <FileText className="w-[16px] h-[16px] text-gray-400" />
                    {category === 'careers' ? 'Job Position' : 'Indexed Fragment'}
                  </div>
                </th>
                <th className="px-4 lg:px-10 py-5 text-body4 font-medium text-gray-500 tracking-tight min-w-[200px]">
                  <div className="flex items-center gap-2.5">
                    <CalendarIcon className="w-[16px] h-[16px] text-gray-400" />
                    {category === 'careers' ? 'Department & Type' : 'Provenance'}
                  </div>
                </th>
                <th className="px-4 lg:px-10 py-5 text-body4 font-medium text-gray-500 tracking-tight text-right min-w-[150px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredItems.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-4 lg:px-10 py-6 lg:py-8">
                    <div className="flex items-center gap-4 lg:gap-6">
                      ...
                      <div className="max-w-xl">
                        <p className="font-medium text-gray-900 text-body4 leading-snug line-clamp-2 group-hover:text-[#FF5C19] transition-colors whitespace-pre-line">
                          {item.title}
                        </p>
                        {category === 'careers' && (
                  ...
                  <td className="px-4 lg:px-10 py-6 lg:py-8">
                    <p className="text-body4 font-medium text-gray-600 tracking-tight truncate max-w-[200px]">
                      {category === 'careers' ? (item.source || 'No Department') : (item.source || '-')}
                    </p>
                    <p className="text-body4 text-gray-400 font-normal mt-1">
                  ...
                  <td className="px-4 lg:px-10 py-6 lg:py-8 text-right">
                    <div className="flex justify-end items-center gap-4 lg:gap-8">
                      <button
                        onClick={() => onEdit(item)}
                        className="text-body4 font-medium text-[#FF5C19] hover:underline cursor-pointer flex items-center gap-2 group/btn"
                      >
                      ...
                      <button
                        ...
                        className="text-body4 font-medium text-red-500 hover:underline cursor-pointer flex items-center gap-2 group/btn"
                      >
          ...
          {filteredItems.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 px-8 text-center bg-gray-50/50">
              ...
              <h3 className="text-gray-900 font-medium tracking-tight">Zero indexed records for {category}</h3>
              <p className="text-gray-400 text-body4 mt-1 max-w-[280px]">
