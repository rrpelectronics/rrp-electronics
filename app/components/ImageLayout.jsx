...
const ImageLayout = ({ heading, imageLayoutData = [] }) => {
  return (
    <section className="h-fit w-full grid grid-cols-4 gap-x-3 md:gap-x-5 px-3.5 md:px-5 lg:px-10 py-10 md:py-15 bg-background">
      {heading && <HeadingCenter heading={heading} />}
      {imageLayoutData.map((imgLayout, id) => (
        <div
          key={id}
          className={`col-span-4 md:col-span-2 ${
            id === 0 ? "mb-6 md:mb-0" : "md:mt-[42.5%]"
          }`}
        >
          <p className="text-heading4 text-black leading-[115%] mb-4 md:mb-5">
            {imgLayout.title}
          </p>
            ...
            {imgLayout.dates && (
              <div className="z-2 h-full w-full absolute left-0 top-0 p-5 lg:p-6 flex items-end bg-black/50">
                <ul className="flex gap-5 items-center justify-end h-fit w-fit">
                  <li className="rounded-full px-2 lg:px-3 py-2 border-1 border-white text-white text-caption leading-[120%] font-neueMontreal">
          ...
          <p className="text-textPrimary font-neueMontreal text-bodySmall leading-[120%] w-[90%] lg:w-[80%]">
